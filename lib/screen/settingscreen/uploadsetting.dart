...
class ImportSetting extends StatefulWidget {
  const ImportSetting({Key? key}) : super(key: key);

  @override
  State<ImportSetting> createState() => _ImportSettingState();
}

class _ImportSettingState extends State<ImportSetting>
  ...
  Future dropzoneresult(ev) async {
    ...
    bloc.add(UploadSettingFile(droppedFile));
  }
  ...
  void uploadSelectedFiles() async {
    ...
    final request = http.MultipartRequest(
      "POST",
      Uri.parse(
          "http://192.168.0.130:9013/temacs/api/temacs/main/v1/uploadSetting"),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<SettingBloc, SettingState>(
      listener: (context, state) {
        ...
        if (state is UploadSettingSuccess) {
          ScaffoldMessenger.of(context)
            ..hideCurrentSnackBar()
            ..showSnackBar(
              const SnackBar(
