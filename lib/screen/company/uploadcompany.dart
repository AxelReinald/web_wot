...
class UploadCom extends StatefulWidget {
  const UploadCom({Key? key}) : super(key: key);

  @override
  State<UploadCom> createState() => _UploadComState();
}

class _UploadComState extends State<UploadCom>
    with SingleTickerProviderStateMixin {
  late DropzoneViewController controller;
  PlatformFile? objFiles = null;
  late CompanyBloc bloc;
  DownloadTemplateDataCom temp = DownloadTemplateDataCom();
  String notif = '';
  double _size = 250.0;
  bool _large = true;
  void _updateSize() {
  ...
  Future dropzoneresult(ev) async {
    ...
    bloc.add(UploadComFile(droppedFile)); //
  }
  ...
  void uploadSelectedFiles() async {
    ...
    final request = http.MultipartRequest(
      "POST",
      Uri.parse(
          "http://192.168.0.130:9017/temacs/api/temacs/main/v1/uploadCompany"),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<CompanyBloc, CompanyState>(
      listener: (context, state) {
        ...
        if (state is TemplateComSuccess) {
          setState(() {
            temp = state.resp.data!;
            writeFileWeb(temp.base64Data.toString(), temp.fileName.toString());
          });
          ;
        }
        if (state is UploadComSuccess) {
          ScaffoldMessenger.of(context)
            ..hideCurrentSnackBar()
            ..showSnackBar(
              const SnackBar(
