...
class SettingGroupBloc extends Bloc<SettingGroupEvent, SettingGroupState> {
  ...
  Uploadresponse uploadresp = Uploadresponse();
      ...
      (event, emit) async {
        try {
          ...
          if (event is InitialScreen) {
            ...
            downresp = await dowloadtemplateresponse(event.downreq);
            emit(DownloadTemplateSuccess(downresp));
  ...
  dowloadtemplateresponse(DownloadRequestSettings req) async {
    DownloadResponseSettingGroup downreq = DownloadResponseSettingGroup();
    downreq = await api.DownloadTemplate(req.toJson());
    return downreq;
  }
